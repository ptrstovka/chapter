<template>
  <CoursesLayout :page-title="$t('Completed')">
    <EmptyState
    ...
    <div v-else class="grid grid-cols-1 gap-4 mt-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <CourseCard v-for="course in completed.data" :course="course" />
    </div>

    <div v-if="completed.total > 0" class="flex flex-row justify-center w-full mt-6">
